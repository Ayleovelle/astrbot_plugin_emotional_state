...
`astrbot_plugin_emotional_state` 是一个 **AstrBot Star 插件**：安装时遵循 AstrBot Star/插件加载规范，运行时提供情绪状态总线、LLM Tools、群聊氛围识别、用户关系轨迹、LivingMemory 写入注解和公开 API。

这次发布页不展开理论依据和长公式。需要跳到仓库或查看实现，请用这个链接：[astrbot_plugin_emotional_state](https://github.com/Ayleovelle/astrbot_plugin_emotional_state)。
...
| 项目 | 值 |
| --- | --- |
| Star 目录 | `astrbot_plugin_emotional_state` |
| 显示名 | `多维情绪状态` |
| 版本 | `0.1.0-exp.1` |
| AstrBot | `>=4.9.2,<5.0.0` |
| Python | `3.10+` |
| license | `GPL-3.0-or-later` |
| repo | `https://github.com/Ayleovelle/astrbot_plugin_emotional_state` |

`metadata.yaml` 中的兼容声明：

```yaml
name: astrbot_plugin_emotional_state
display_name: 多维情绪状态
version: 0.1.0-exp.1
license: GPL-3.0-or-later
repo: "https://github.com/Ayleovelle/astrbot_plugin_emotional_state"
...
从 GitHub Release 下载 `astrbot_plugin_emotional_state.zip`，在 AstrBot WebUI 的 `插件` / `Star` 页面点击 `+`，选择文件上传。
...
上传前 zip 应该长这样，顶层必须是 `astrbot_plugin_emotional_state/`，不能把文件平铺在 zip 根目录：
...
git clone https://github.com/Ayleovelle/astrbot_plugin_emotional_state.git astrbot_plugin_emotional_state
```
...
`scripts\plugin_zip_preflight.js` 会读取 central directory、校验 `metadata.yaml` 的 `name:`、确认 zip 位于 `astrbot_plugin_emotional_state/` 根目录下，并拒绝 `tests`、`scripts`、`output`、`dist`、`raw`、`.git`、`__pycache__` 等目录。
...
上传脚本只允许调用 `/api/plugin/install-upload`，失败上传目录只允许通过 `uninstall-failed` 清理，且必须 `delete_config=false`、`delete_data=false`。如果出现 `plugin_upload_astrbot_plugin_emotional_state`，可用清理脚本处理；清理脚本必须带：
...
清理脚本只允许处理 `astrbot_plugin_emotional_state` 和 `plugin_upload_astrbot_plugin_emotional_state`，不会触碰 LivingMemory 或其他插件。
...
| 上传 zip 后不显示 | 检查 zip 顶层是否是 `astrbot_plugin_emotional_state/`，不要上传 GitHub 的 source code zip |
